...
#  VARIAVEIS
ip_server = "191.52.64.154"
port_server = 8080

clients = {}
directClients = {}
clientsList = ip_server + "-" + (str(port_server)) + "-server" #Inicializa uma string que armazenara os clientes no formato (@ip-port-nome)
clientsListAddresses = ip_server + "-" + (str(port_server)) + "-" + (str(port_server)) + "-server" #Inicializa uma string que armazenara os enderecos dos clientes no formato (@ip-port_cliente-port_servidor])
...
def delete_element_clientsListAddresses(ip, port):
    ...
    for x in List:
        ...
        if ip == ipAtual and port == portAtual:
            print("\nCliente " +clientInfo[3] + "(" + ip + "," + (str(portAtual)) + ") se desconectou")
        else:
            if auxList == "0":
...
#Repassa mensagem sem repetir aos mesmos clientes
